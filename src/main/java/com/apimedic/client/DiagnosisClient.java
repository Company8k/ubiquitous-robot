...
import com.apimedic.model.AccessToken;
import com.apimedic.model.DiagnosedSpecialisation;
import com.apimedic.model.Gender;
import com.apimedic.model.HealthDiagnosis;
import com.apimedic.model.HealthIssueInfo;
import com.apimedic.model.HealthItem;
import com.apimedic.model.HealthSymptomSelector;
import com.apimedic.model.SelectorStatus;
...
public class DiagnosisClient {
    ...
    private void LoadToken(String username, String password, String url) throws Exception {
        ...
        Mac mac = HmacUtils.getInitializedMac(HmacAlgorithms.HMAC_MD5, password.getBytes());
        byte[] result = mac.doFinal(url.getBytes());

        String computedHashString = Base64.encodeBase64String(result);
    ...
    private <T> T loadFromWebService(String action, TypeReference< ? > valueTypeRef) throws Exception {
        ...
        String url =
                     new StringBuilder(this.healthServiceUrl).append("/").append(action).append(action.contains("?") ? "&" : "?")
    ...
    public List<DiagnosedSpecialisation> loadSpecialisations(List<Integer> selectedSymptoms, Gender gender, int yearOfBirth) throws Exception {
        ...
        String serializedSymptoms = new ObjectMapper().writeValueAsString(selectedSymptoms);
        String action =
                        "diagnosis/specialisations?symptoms=" + serializedSymptoms + "&gender=" + gender.toString() + "&year_of_birth="
                            + yearOfBirth;
    ...
    public List<HealthItem> loadProposedSymptoms(List<Integer> selectedSymptoms, Gender gender, Integer yearOfBirth) throws Exception {
        ...
        String serializedSymptoms = new ObjectMapper().writeValueAsString(selectedSymptoms);
        String action =
                        "symptoms/proposed?symptoms=" + serializedSymptoms + "&gender=" + gender.toString() + "&year_of_birth="
                            + yearOfBirth;
